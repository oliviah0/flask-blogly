...
class FlaskTest(TestCase):
    ...
    def setUp(self):
        ...
        post2 = Post(title="Post example 2",
                     content=LOREM_IPSUM.upper(), user_id=2)
    ...
    def test_users(self):
        """Tests the users page (all users)"""
    ...
    def test_create(self):
        ...
        with self.client:
            response = self.client.post('/users/create',
                                        data={"first-name": "Cactus",
                                              "last-name": "Prickle",
                                              "img-url": "https://cdn.shopify.com/s/files/1/1111/4656/products/20181115_125634_600x800_600x.jpg?v=1543815335"}, 
    ...
    def test_edit(self):
        ...
        with self.client:
            ...
            self.assertEqual(response.status_code, 200)
            # response data returns HTML or whatever we return in app.py
    ...
    def test_post_page(self):
        ...
        with self.client:
            ...
            self.assertIn(
                b'<a href="/users/user/1" class="btn btn-outline-primary">Cancel</a>', response.data)
            self.assertIn(
                b'<a href="/posts/1/edit" class="btn btn-primary">Edit</a>', response.data)
            self.assertIn(
                b'<a href="/posts/1/delete" class="btn btn-danger">Delete</a>', response.data)

    def test_edit_post(self):
        """Test editing a post """

        with self.client:
            response = self.client.post('/posts/2/edit',
                                        data={
                                            "post-title": "WE EDIT POST #2", 
                                            "post-content": LOREM_IPSUM[::-1]}, 
                                            follow_redirects=True)

            self.assertIn(b'WE EDIT POST #2', response.data)
    ...
    def test_delete_post(self):
        ...
        with self.client:
            ...
            self.assertNotIn(b'Post example 1', response.data)
